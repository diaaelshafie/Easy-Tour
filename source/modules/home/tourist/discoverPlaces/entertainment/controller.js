...
export const getAllPlacesData = async (req, res, next) => {
    console.log("\nSTATIC GET ALL ENTERTAINMENT PLACES API\n")
    const { type } = req.body
...
// TODO : make the toggle favourites here also
export const placeToggleFav = async (req, res, next) => {
    ...
    const getPlace = await entertainmentMPmodel.findOne({ name: placeName })
    if (!getPlace) {
    ...
    if (req.authUser.favouriteEntertainmentPlaces) {
        // if the place was already in the tourist's favourits -> remove it
        if (req.authUser.favouriteEntertainmentPlaces.includes(getPlace._id)) {
            console.log({ message: "place is in user favs, removing it..." })
            req.authUser.favouriteEntertainmentPlaces = req.authUser.favouriteEntertainmentPlaces.filter(placeId => placeId.toString() !== getPlace._id.toString())
            console.log({ message: "place is removed from the favourites" })
        }
        // if the place is not in the user favs -> add it
        else if (!req.authUser.favouriteEntertainmentPlaces.includes(getPlace._id)) {
            console.log({ message: "place is not in the user favs , adding it..." })
            req.authUser.favouriteEntertainmentPlaces.push(getPlace._id)
            console.log({ message: "place added to the user favs successfully" })
        }
    } else { // if the user had no favouriteEntertainmentPlaces field -> add one in it
    ...
    res.status(StatusCodes.OK).json({
        ...
        favourites: req.authUser.favouriteEntertainmentPlaces
    })
}
